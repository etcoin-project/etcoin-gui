; ETcoin Nitrogen Nebula GUI Wallet Installer for Windows
; Copyright (c) 2017-2020, The Monero Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\etcoin-wallet-gui.exe")

[Setup]
AppName=ETcoin GUI Wallet
...
DefaultDirName={commonpf}\ETcoin GUI Wallet
DefaultGroupName=ETcoin GUI Wallet
UninstallDisplayIcon={app}\etcoin-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The ETcoin Developer Community
AppPublisherURL=https://etcoin.xyz
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the ETcoin files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to ETcoin.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; ETcoin GUI wallet exe and guide
Source: "bin\etcoin-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\etcoin-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; ETcoin CLI wallet
Source: "bin\extras\etcoin-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\etcoin-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; ETcoin wallet RPC interface implementation
Source: "bin\extras\etcoin-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; ETcoin daemon
Source: "bin\etcoind.exe"; DestDir: "{app}"; Flags: ignoreversion

; ETcoin daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "etcoin-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; ETcoin blockchain utilities
Source: "bin\extras\etcoin-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\etcoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\etcoin-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\etcoin-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\etcoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\etcoin-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\etcoin-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\etcoin-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\etcoin-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\etcoin-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\etcoin-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\etcoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\etcoin');
  s := 'The default folder to store the ETcoin blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\etcoin-project\etcoin-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" ETcoin default location
...
begin
  Result := BlockChainDir('') + '\etcoin.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "etcoin-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the ETcoin daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\etcoind.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\etcoin-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "ETcoin GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\etcoin-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\etcoin-gui-wallet-guide.pdf"; IconFilename: "{app}\etcoin-wallet-gui.exe"
...
Name: "{group}\Utilities\ETcoin Daemon"; Filename: "{app}\etcoind.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\etcoin-wallet-cli.exe"; WorkingDir: "{userdocs}\ETcoin\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\ETcoin\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\etcoin-wallet-gui\etcoin-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\etcoin-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM etcoind.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\etcoin-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\etcoin-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\etcoin-project\etcoin-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\etcoin-project\etcoin-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "etcoin:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "etcoin://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "etcoin"; ValueType: "string"; ValueData: "URL:ETcoin Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "etcoin"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "etcoin\DefaultIcon"; ValueType: "string"; ValueData: "{app}\etcoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "etcoin\shell\open\command"; ValueType: "string"; ValueData: """{app}\etcoin-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "etcoinseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "etcoinseed://a.b.c.d:port"
Root: HKCR; Subkey: "etcoinseed"; ValueType: "string"; ValueData: "URL:ETcoin Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "etcoinseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "etcoinseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\etcoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "etcoinseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\etcoin-wallet-gui.exe"" ""%1"""
