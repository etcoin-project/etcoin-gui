...
function walletPathExists(directory, filename, isIOS, walletManager) {
    ...
    if(isIOS)
        var path = etcoinAccountsDir + filename;
    else
...
function getApproximateBlockchainHeight(_date, _nettype){
    // Monday, June 29, 2020 3:00:14 AM  1593399614
    var etcoinBirthTime = _nettype == "Mainnet" ? 1593399614 : _nettype == "Testnet" ? 1410295020 : 1518932025;
    // avg seconds per block in v1
    var secondsPerBlockV1 = 60;
    // time of v2 fork Monday, June 29, 2020 3:47:14 AM 
    var forkTime = _nettype == "Mainnet" ? 1593402434 : _nettype == "Testnet" ? 1448285909 : 1520937818;
    // v2 fork block
    var forkBlock = _nettype == "Mainnet" ? 20 : _nettype == "Testnet" ? 624634 : 32000;
    ...
    var secondsPerBlock;
    // before etcoin's birth
    if (requestedTime < etcoinBirthTime){
        console.log("Calculated blockchain height: 0, requestedTime < etcoinBirthTime " );
        return 0;
    }
    // time between during v1
    if (requestedTime > etcoinBirthTime && requestedTime < forkTime){
        approxBlockchainHeight = Math.floor((requestedTime - etcoinBirthTime)/secondsPerBlockV1);
