...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/etcoin-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    ETCOIND_EXEC=etcoind.exe
fi
...
get_tag
popd
echo "var GUI_ETCOIN_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../etcoin-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy etcoind to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$ETCOIN_DIR/bin/$ETCOIND_EXEC $BIN_PATH
fi
