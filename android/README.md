...
# Base environnement
        ...
        cd etcoin/utils/build_scripts
        docker build -f android32.Dockerfile -t etcoin-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t etcoin-gui-android .
        docker create -it --name etcoin-gui-android etcoin-gui-android bash

# Get the apk

        docker cp etcoin-gui-android:/opt/android/etcoin-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/etcoin-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
